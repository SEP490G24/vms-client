...
const Module: React.FC<ModuleArgs> = (args) => {
  ...
  useEffect(() => {
    setFeatures(groupBy(args.module.permissionDtos, permission => permission.label[i18n.language]?.feature ?? ''))
  }, [args, i18n.language])
    ...
    <ModuleWrapper>
      <Space size={24} direction={'vertical'} className={'w-full'}>
        {Array.from(features?.entries() ?? []).map((entry, index) => {
          return (<FeaturePermission key={index} title={entry[0]} permissions={entry[1]} roles={args.roles} onSave={args.onSave}
